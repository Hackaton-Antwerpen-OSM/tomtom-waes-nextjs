...
const Map: React.FC<MapProps> = ({ 
  ...
  useEffect(() => {
        ...
        if (leafletMapRef.current && userLocation) {
          ...
          pointsOfInterest.forEach(poi => {
            ...
            L.marker([poi.latitude, poi.longitude], { 
              ...
              .bindPopup(`<b>${poi.name}</b><br>${poi.type}<br>${poi.distance}m away`);
          });
            ...
            if (window.L && window.L.Routing) {
              ...
              if (window.L.Routing.osrmv1) {
                ...
                routingControlRef.current = window.L.Routing.control({
                  ...
                  altLineOptions: {
                    styles: [
                      { color: '#64748b', opacity: 0.4, weight: 4 }
                    ]
                  },
                  router: window.L.Routing.osrmv1(routerConfig),
                  createMarker: function() { return null; } // We already have our own markers
                }).addTo(leafletMapRef.current);
              } else {
                console.error("Leaflet Routing Machine OSRM router not loaded correctly");
              }
            }
    ...
    return () => {
      ...
      const routingCSS = document.querySelector('link[href*="leaflet-routing-machine.css"]');
      if (routingCSS) {
        document.head.removeChild(routingCSS);
      }
      // Also remove the script
      const routingScript = document.querySelector('script[src*="leaflet-routing-machine.js"]');
      if (routingScript) {
        document.head.removeChild(routingScript);
      }
    };
  }, [userLocation, pointsOfInterest, selectedPOI, showDirections, transportMode]);

  return (
    <div ref={mapRef} className="h-60 w-full rounded-md overflow-hidden shadow-md" />
  );
};
