 ...
 */
export async function getNearbyPointsOfInterest(location: Location, initialRadius: number = 500): Promise<PointOfInterest[]> {
  // Try with increasing radius until we find something or hit max radius
  const maxRadius = 5000; // 5km max search radius
  const radiusStep = 500; // Increase by 500m each time
  ...
  while (radius <= maxRadius) {
    try {
      ...
      if (pois.length > 0) {
        console.log(`Found ${pois.length} points of interest within ${radius}m`);
        break;
      }
 ...
 */
async function fetchPointsOfInterest(location: Location, radius: number): Promise<PointOfInterest[]> {
  try {
    ...
    const overpassQuery = `
      [out:json];
      (
        node["shop"](around:${radius},${location.latitude},${location.longitude});
        ...
        node["leisure"="park"](around:${radius},${location.latitude},${location.longitude});
        node["leisure"="garden"](around:${radius},${location.latitude},${location.longitude});
        node["leisure"="nature_reserve"](around:${radius},${location.latitude},${location.longitude});
        ...
        node["amenity"="pub"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="library"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="theatre"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="cinema"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="arts_centre"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="bar"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="fast_food"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="fountain"](around:${radius},${location.latitude},${location.longitude});
        node["amenity"="nightclub"](around:${radius},${location.latitude},${location.longitude});
        node["natural"="beach"](around:${radius},${location.latitude},${location.longitude});
        node["natural"="cliff"](around:${radius},${location.latitude},${location.longitude});
        node["natural"="peak"](around:${radius},${location.latitude},${location.longitude});
        node["natural"="water"](around:${radius},${location.latitude},${location.longitude});
        node["natural"="bay"](around:${radius},${location.latitude},${location.longitude});
        node["natural"="spring"](around:${radius},${location.latitude},${location.longitude});
        node["man_made"="tower"](around:${radius},${location.latitude},${location.longitude});
        node["man_made"="bridge"](around:${radius},${location.latitude},${location.longitude});
        node["man_made"="lighthouse"](around:${radius},${location.latitude},${location.longitude});
        node["man_made"="windmill"](around:${radius},${location.latitude},${location.longitude});
        node["man_made"="water_tower"](around:${radius},${location.latitude},${location.longitude});
        node["man_made"="pier"](around:${radius},${location.latitude},${location.longitude});
        node["memorial"](around:${radius},${location.latitude},${location.longitude});
      );
      out body;
    `;
    ...
    const pois: PointOfInterest[] = data.elements.map((element: any) => {
      ...
      return {
        ...
        name: element.tags?.name || element.tags?.operator || `${poiType} at ${Math.round(distance)}m`,
        type: poiType,
    ...
    return pois
      .filter(poi => {
        ...
        if (poi.type === 'amenity' && !VALUABLE_AMENITIES.includes(poi.type)) return false;
...
export async function getWikipediaInfo(poi: PointOfInterest): Promise<string> {
  try {
    ...
    if (!searchData.query.geosearch || searchData.query.geosearch.length === 0) {
